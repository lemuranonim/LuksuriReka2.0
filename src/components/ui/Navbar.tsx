...
export default function Navbar() {
    ...
    return (
        <nav className="fixed top-0 left-0 w-full p-6 flex justify-between items-center z-50 pointer-events-auto text-white bg-[#0A2540]/70 backdrop-blur-md border-b border-white/10 md:bg-transparent md:backdrop-blur-none md:border-none">
            <button
                onClick={() => {
            ...
            <button
                ...
                className={`hidden md:flex items-center gap-2 px-4 py-2 rounded-full font-mono text-sm transition-all duration-300 ${isDevMode ? 'bg-neon-cyan text-trust-blue shadow-[0_0_15px_rgba(0,240,255,0.5)]' : 'bg-white/10 text-white hover:bg-white/20 backdrop-blur-md border border-white/20'}`}
            >
