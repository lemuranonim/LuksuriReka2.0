  ...
  const handleExport = async (type: 'jpg' | 'pptx' | 'pdf') => {
    ...
    try {
      if (type === 'jpg') {
        ...
        forceDownload(blob, 'WA-Integration-Proposal-Slides.zip');
      } else if (type === 'pptx') {
        const blob = await exportToPptx(totalSlides);
        forceDownload(blob, 'WA-Integration-Proposal.pptx');
      } else if (type === 'pdf') {
        const blob = await exportToPdf(totalSlides);
        forceDownload(blob, 'WA-Integration-Proposal.pdf');
      }
    } catch (err) {
    ...
    >
      ...
      {Array.from({ length: totalSlides }).map((_, index) => {
        ...
        return (
          <iframe
            key={slideNum}
            src={`/reports/wa-proposal/slide${slideNum}.html`}
      ...
      >
        ...
        {/* Kotak Loading */}
        <div className="relative flex flex-col items-center bg-slate-800/60 border border-emerald-500/30 p-8 rounded-2xl shadow-[0_0_50px_rgba(16,185,129,0.2)]">
          <div className="w-16 h-16 border-4 border-slate-600 border-t-emerald-500 rounded-full spin mb-6"></div>
      ...
      `}>
        <button
          ...
          disabled={!!exporting}
          className="flex items-center gap-2 bg-slate-900/80 hover:bg-emerald-600 text-white border border-emerald-500/50 backdrop-blur-md px-4 py-2 rounded-lg text-sm font-semibold transition-all disabled:opacity-50"
        >
      ...
      `}>
        <button
          ...
          className="w-10 h-10 flex items-center justify-center rounded-full bg-slate-800 text-white hover:bg-emerald-500 disabled:opacity-30 transition-all cursor-pointer"
        >
