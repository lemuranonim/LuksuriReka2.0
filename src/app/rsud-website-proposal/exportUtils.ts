/**
 * Export utilities untuk RSUD Website Proposal Presentation
...
async function captureSlideUrl(slideNumber: number): Promise<string | null> {
    return new Promise((resolve) => {
        ...
        iframe.src = `/reports/rsud-website-proposal/slide${slideNumber}.html`;
            ...
            try {
                ...
                styleInject.innerHTML = `
                    ...
                    * {
                    ...
                    .glass-card, .module-card, .highlight-card, .advantage-card,
                    .conclusion-card, .cta-section, .infra-item, .summary-card,
                    .score-box, .phase-content {
...
export async function exportToJpgZip(totalSlides: number): Promise<Blob | null> {
    ...
    const zip = new JSZip();
    const folder = zip.folder('RSUD-Website-Proposal-Slides')!;
