...
export default function RSUDProposalViewer() {
  ...
  const handleExport = async (type: 'jpg' | 'pptx' | 'pdf') => {
    ...
    try {
      if (type === 'jpg') {
        ...
        forceDownload(blob, 'RSUD-Website-Proposal-Slides.zip');
      } else if (type === 'pptx') {
        const blob = await exportToPptx(totalSlides);
        forceDownload(blob, 'RSUD-Website-Proposal.pptx');
      } else if (type === 'pdf') {
        const blob = await exportToPdf(totalSlides);
        forceDownload(blob, 'RSUD-Website-Proposal.pdf');
      }
    } catch (err) {
  ...
  const handleLogin = (e: React.FormEvent) => {
    e.preventDefault();
    if (tokenInput.trim().toUpperCase() === 'RSUDISKAK2026') {
  ...
  if (!isAuthenticated) {
    return (
      <div className="w-screen h-screen bg-[#050510] flex items-center justify-center font-sans overflow-hidden relative">
        ...
        <div className="relative z-10 bg-slate-800/80 border border-sky-500/30 p-10 rounded-2xl shadow-[0_0_50px_rgba(14,165,233,0.2)] max-w-sm w-full mx-4 text-center">
          <div className="w-20 h-20 bg-sky-500/20 rounded-full flex items-center justify-center mx-auto mb-6">
            <i className="fa-solid fa-hospital text-4xl text-sky-400"></i>
          </div>
          <h2 className="text-2xl font-bold text-white mb-2">Secure Access</h2>
          <p className="text-slate-400 text-sm mb-6">Masukkan Security Token untuk melihat Proposal Website RSUD Dr. Iskak.</p>
          ...
          <form onSubmit={handleLogin} className="flex flex-col gap-4">
            <div>
              <input
                ...
                className="w-full bg-slate-900/80 border border-slate-600 rounded-lg px-4 py-3 text-white text-center tracking-widest focus:outline-none focus:border-sky-500 transition-colors"
              />
            ...
            <button type="submit" className="w-full bg-sky-600 hover:bg-sky-500 text-white font-bold py-3 rounded-lg transition-colors flex items-center justify-center gap-2">
    ...
    >
      ...
      {Array.from({ length: totalSlides }).map((_, index) => {
        ...
        return (
          <iframe
            key={slideNum}
            src={`/reports/rsud-website-proposal/slide${slideNum}.html`}
      ...
      >
        ...
        <div className="relative flex flex-col items-center bg-slate-800/60 border border-sky-500/30 p-8 rounded-2xl shadow-[0_0_50px_rgba(14,165,233,0.2)]">
          <div className="w-16 h-16 border-4 border-slate-600 border-t-sky-500 rounded-full spin mb-6"></div>
      ...
      `}>
        <button
          ...
          disabled={!!exporting}
          className="flex items-center gap-2 bg-slate-900/80 hover:bg-sky-600 text-white border border-sky-500/50 backdrop-blur-md px-4 py-2 rounded-lg text-sm font-semibold transition-all disabled:opacity-50"
        >
      ...
      `}>
        <button
          ...
          className="w-10 h-10 flex items-center justify-center rounded-full bg-slate-800 text-white hover:bg-sky-500 disabled:opacity-30 transition-all cursor-pointer"
        >
        ...
        <button
          ...
          disabled={!!exporting}
          className="w-10 h-10 flex items-center justify-center rounded-full bg-slate-800 text-slate-300 hover:text-white hover:bg-sky-500 disabled:opacity-30 transition-all cursor-pointer"
        >
          <i className={`fa-solid ${isFullscreen ? 'fa-compress' : 'fa-expand'}`}></i>
        </button>
      </div>

      {/* Area klik navigasi saat UI tersembunyi */}
